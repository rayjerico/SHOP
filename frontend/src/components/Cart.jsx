...
const Cart = () => {
  ...
  const updateQuantity = async (cartItem, newQty) => {
    ...
    if (newQty > stockCount) {
      setError(`Only ${stockCount} item(s) are available in stock.`);
      return;
    }
  ...
  const removeCartItem = async (cartItemId) => {
    try {
      ...
      setMessage(response?.data?.detail || "Product removed from cart.");

      await fetchCart();
    } catch (err) {
      console.error("Failed to delete cart item:", err);

      setError(getErrorMessage(err, "Failed to remove product from cart."));
    } finally {
      setDeletingId(null);
    }
  };
  ...
  const getProductImage = (image) => {
    ...
    // DRF may already return a complete URL
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="mt-2 flex items-center gap-2 text-xs">
                          ...
                          <button
                            ...
                            disabled={item.qty >= stockCount || isUpdating}
                            className="flex h-6 w-6 items-center justify-center rounded-full bg-gray-100 shadow-sm transition hover:bg-gray-200 disabled:cursor-not-allowed disabled:opacity-40"
                          >
                            +
                          </button>
